...
class NDPITileCropperCLI(object):
    ...
    def _create_parser():
        ...
        # Jonathan edit for tiling only particular region of the ndpi
        parser.add_argument(
            '--region_start_x', '-rsx',
            type=int,
            nargs='?',
            help="starting x coordinate of some specified region"
        )
        parser.add_argument(
            '--region_end_x', '-rex',
            type=int,
            nargs='?',
            help="ending x coordinate of some specified region"
        )
        parser.add_argument(
            '--region_start_y', '-rsy',
            type=int,
            nargs='?',
            help="starting y coordinate of some specified region"
        )
        parser.add_argument(
            '--region_end_y', '-rey',
            type=int,
            nargs='?',
            help="ending y coordinate of some specified region"
        )
        return parser
...
class NDPIFileCropper:
    ...
    def __init__(self, input_file, output_dir=None, tile_size=1024, tile_overlap=0, tile_format='png', overwrite=False,
                 zip_flag=False, region_start_x=None, region_end_x=None, region_start_y=None, region_end_y=None):
        ...
        self.region_start_x = region_start_x
        self.region_end_x = region_end_x
        self.region_start_y = region_start_y
        self.region_end_y = region_end_y
        self.metadata = dict()
    ...
    def crop_tiles(self):
        ...
        start_x_list = np.arange(self.region_start_x if self.region_start_x is not None else 0, 
                              self.region_end_x if self.region_end_x is not None else self.metadata['width'], 
                              width - overlap).tolist()
        start_y_list = np.arange(self.region_start_y if self.region_start_y is not None else 0, 
                              self.region_end_y if self.region_end_y is not None else self.metadata['height'], 
                              height - overlap).tolist()
        start_xy_list = []
...
if __name__ == '__main__':
    ...
    # log_file = 'ndpi_tile_cropper.log'
    logging.basicConfig(
        format='%(asctime)s %(levelname)-7s : %(name)s - %(message)s',
        level=cli.args.log_level
        # handlers=[
    ...
    ndpi_file_cropper = NDPIFileCropper(cli.args.input_file, cli.args.output_dir, cli.args.tile_size,
                                        cli.args.tile_overlap, cli.args.tile_format, cli.args.overwrite, 
                                        region_start_x=int(cli.args.region_start_x), region_end_x=int(cli.args.region_end_x), region_start_y=int(cli.args.region_start_y), region_end_y=int(cli.args.region_end_y))
